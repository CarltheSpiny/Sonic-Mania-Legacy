...
public class MCreatorPGZLog extends Elementssonic_mania.ModElement {
	@ObjectHolder("sonic_mania:pgzlog")
	...
	public static class CustomBlock extends Block {
		...
		public VoxelShape getShape(BlockState state, IBlockReader world, BlockPos pos, ISelectionContext context) {
			switch ((Direction) state.get(FACING)) {
				...
				case NORTH :
				default :
					return VoxelShapes.create(0.4D, 0D, 0.4D, 0.6D, 1D, 0.6D);
				case EAST :
				case WEST :
					return VoxelShapes.create(0.4D, 0.6D, 0D, 0.6D, 0.4D, 1D);
				case UP :
				case DOWN :
					return VoxelShapes.create(0D, 0.6D, 0.6D, 1D, 0.4D, 0.4D);
			}
		}
